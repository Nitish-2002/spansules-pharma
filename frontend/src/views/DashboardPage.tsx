    ...
    <div className="space-y-8 max-w-7.5xl mx-auto animate-fade-in">
      ...
      <div className="relative overflow-hidden rounded-3xl p-8 sm:p-10 text-white shadow-xl premium-shadow" style={{ backgroundColor: 'var(--primary-color)' }}>
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          <div>
            <span className="px-3.5 py-1.5 bg-white/10 backdrop-blur-md rounded-full text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-white/10">
              Enterprise Dashboard
            </span>
            <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight mb-3 leading-tight">{t('welcomeTitle')}</h2>
            <p className="text-white/80 text-sm sm:text-base max-w-2xl font-light leading-relaxed">
              {t('welcomeSubtitle')}
            </p>
          </div>
          <div className="bg-white/10 backdrop-blur-xl px-8 py-6 rounded-3xl border border-white/10 text-center shadow-2xl min-w-[220px]">
            <span className="block text-[10px] uppercase tracking-widest font-semibold text-white/70 mb-2">{t('totalSalesRevenue')}</span>
            <span className="text-4xl font-black text-white tracking-tight">${salesAnalytics.totalSales || '525,800'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="glass-card p-6 sm:p-8 rounded-3xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <h3 className="text-xl font-bold text-gray-800 tracking-tight">{t('revenueTrend')}</h3>
